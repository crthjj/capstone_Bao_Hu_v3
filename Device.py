...
class GPSCar(trafficDevice):
    ...
    def dealGPSMessage(self):
        ...
        for i in range(self.__curPosInPath,len(self.__carPath)):
            _cordinateStr = self.__carPath[i]
            _posOfComma = int(_cordinateStr.index(','))
        ...
        if ismapchange==True:
            for i in range(self.__mapSize):
                for j in range(self.__mapSize):
                    ...
                    for k in range(len(_gpsmap[i][j])):
                        self.__myMap[i][j].addRoadDirecs(int(_gpsmap[i][j][k]))
            self.__recalculatePath()
        else:
            return
